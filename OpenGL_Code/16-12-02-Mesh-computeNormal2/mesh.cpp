...
void CMesh::computeNormals() {
	for (int i = 0; i < nF; i++){
		...
		norms[v0].x += normal.x; norms[v1].x += normal.x; norms[v2].x += normal.x;
		norms[v0].y += normal.y; norms[v1].y += normal.y; norms[v2].y += normal.y;
		norms[v0].z += normal.z; norms[v1].z += normal.z; norms[v2].z += normal.z;
	}

	for (int i = 0; i < nV; i++){
		float len = sqrt(norms[i].x*norms[i].x + norms[i].y*norms[i].y + norms[i].z*norms[i].z);
		norms[i].x /= len; // 각각의 법선 벡터들은 정규화가 됨
		norms[i].y /= len;
		norms[i].z /= len;
	}
}
...
void CMesh::read(char *fname) {
	...
	for (int i = 0; i < nV; i++) {
		...
		norms[i].x = norms[i].y = norms[i].z = 0;
	}
	fscanf(f, "%d", &nF);
