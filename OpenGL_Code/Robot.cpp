...
GLint Cameranum = 0, Boxup = -1, Boxx = 0, Boxy = 3, Boxz = 0;
...
void  special(int key, int x, int y)
{
	...
	case GLUT_KEY_UP:
		...
		else if (headrotate == 225){
			movex -= 0.5; movez += 0.5;
		}
		else if (headrotate == 270){
			movex -= 0.5;
		}
		else if (headrotate == 315){
			movex -= 0.5; movez -= 0.5;
		}
		else if (headrotate == 0){
			movez -= 0.5;
		}
		else if (headrotate == 45){
			movex += 0.5; movez -= 0.5;
		}
		else if (headrotate == 90){
			movex += 0.5;
		}
		else if (headrotate == 135){
			movex += 0.5; movez += 0.5;
		}
		// 머리 업다운
		headupdown -= 3;
...
void myDisplay() {
	...
	glLoadIdentity();
	switch (Cameranum){
	case 0: gluLookAt(1, 0, -1, 0, 1, 0, 0, 1.5, 0); break; // 대각선
	...
	if (Boxup == -1){
		...
		glRotatef(Boxx, 0, 1, 0);
		glTranslatef(0, 0, Boxz);
	}
	glColor3f(0, 0.7, 0);
