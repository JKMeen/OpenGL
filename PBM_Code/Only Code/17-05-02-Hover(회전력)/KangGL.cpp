...
// Constructor
CKangGL::CKangGL() : bStarted(false), bPaused(false), plane(NULL), nPoints(0), nSpheres(0), nHovers(0) {
	mEx = 1;
	mEy = 1;
	mEz = 1;
	mTx = 0;
...
void CKangGL::switchEngine(int engineNumber){
	for (int i = 0; i < nHovers; i++){
...
void keyboard(unsigned char c, int x, int y) {
	...
	case 'l': win->mEx++;  break; // 시점 변경
	case 'k': win->mEx--;  break;
	case '1': win->switchEngine(0); break; // 호버크래프트 
	case '2': win->switchEngine(1); break;
	case '3': win->switchEngine(2); break;
	case '4': win->switchEngine(3); break;
	}
}
