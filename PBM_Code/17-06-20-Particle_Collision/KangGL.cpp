...
// Constructor
CKangGL::CKangGL() : bStarted(false), bPaused(false), plane(NULL), cloth(NULL) {
	mEx = 1;
	mEy = 1;
	mEz = 1;
	mTx = 0;
...
void CKangGL::addCloth(CCloth *clothObj) {
	if (this->cloth) delete cloth;
	cloth = clothObj;
}
...
void keyboard(unsigned char c, int x, int y) {
	...
	case 'l': win->mEx++; win->mTx++; break;
	case 'j': win->mEx--; win->mTx--; break;
	case 'k': win->mEz++; win->mTz++; break;
	case 'i': win->mEz--; win->mTz--; break;
	case 'u': win->mEy--; win->mTy--; break;
	case 'o': win->mEy++; win->mTy++; break;
	}
}
