...
w(nCol), h(nRow)
{
	...
	for (int i = 0; i < nPart; i++) {
		float x = rand() % w;//(i % w)*xStep;
		float y = rand() % h;//(i / w)*yStep;
		loc[i].set(x, 5, y);
		vel[i].set(0, 0, 0);
		force[i].set(0, 0, 0);
		normal[i].set(0, 1, 0);
		color[i].set(rand() % 100 / 101, rand() % 100 / 101, rand() % 100 / 101);
		mass[i] = _mass / nPart;
	}
...
void CCloth::simulate(float dt) {
	...
	for (int i = 0; i < nPart; i++) {
		vec3d relWind = wind + vec3d(rand() % 10 - 5, 0, 0) - vel[i]; // 속도를 빼주면 상대 바람
		vec3d windDir = relWind.getNormalized();
		float DOT = dot(relWind, normal[i]);
...
CConfetti::CConfetti(int nRow, int nCol, float width, float height, float _mass, float _k)
{
	w = nCol;
	h = nRow;
	nPart = w * h;
